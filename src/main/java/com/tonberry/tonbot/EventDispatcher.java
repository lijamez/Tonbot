 ...
 * A set of activities is registered with this dispatcher. Each activity has a route.
 * This class will check the message event's contents, tokenize the words, and attempt to find the best activity to run.
 * The best activity is the one that has the longest token prefix.
 */
class EventDispatcher {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        for (Activity activity : activities) {
            ...
            if (isPrefix(route, remainingTokens) && (bestActivity == null || bestActivity.getDescriptor().getRoute().size() < route.size())) {
